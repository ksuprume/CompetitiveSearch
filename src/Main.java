...
import Extension.ConfigurableWorldParameters;
...
public class Main {
	...
	public static void main(String[] args) throws IOException, ClassNotFoundException {
		...
		ConfigurableWorldParameters params = new ConfigurableWorldParameters();
		String configFile = "etc/config.properties";
		if(args.length>=1 && args[0].endsWith(".properties")) {
			configFile = args[0];
			 try {
				params = new ConfigurableWorldParameters(configFile);
			} catch (Exception e) {
			}
			 ConfigurableWorldParameters.instance = params;
		}
		try {
